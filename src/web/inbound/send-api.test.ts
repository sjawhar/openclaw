...
describe("createWebSendApi", () => {
  ...
  const onWhatsApp = vi.fn(async () => [{ exists: true, jid: "1555@s.whatsapp.net" }]);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    onWhatsApp.mockResolvedValue([{ exists: true, jid: "1555@s.whatsapp.net" }]);
  });
  ...
  it("sends plain text messages", async () => {
    ...
    expect(onWhatsApp).toHaveBeenCalledWith("1555@s.whatsapp.net");
  ...
  it("fails fast when a direct target is not registered on WhatsApp", async () => {
    onWhatsApp.mockResolvedValueOnce([{ exists: false }]);
    await expect(api.sendMessage("+1555", "hello")).rejects.toThrow(
      /WhatsApp target is not registered: \+1555/,
    );
    expect(sendMessage).not.toHaveBeenCalled();
  });

  it("skips onWhatsApp validation for group JIDs", async () => {
    await api.sendMessage("12345@g.us", "hello");
